@extends('layouts.admin')
@section('content')

<div class="card">
    <div class="card-header">
        {{ trans('global.show') }} {{ trans('cruds.workImage.title') }}
    </div>

    <div class="card-body">
        <div class="form-group">
            <div class="form-group">
                <a class="btn btn-default" href="{{ route('admin.work-images.index') }}">
                    {{ trans('global.back_to_list') }}
                </a>
            </div>
            <table class="table table-bordered table-striped">
                <tbody>
                    <tr>
                        <th>
                            {{ trans('cruds.workImage.fields.id') }}
                        </th>
                        <td>
                            {{ $workImage->id }}
                        </td>
                    </tr>
                    <tr>
                        <th>
                            {{ trans('cruds.workImage.fields.work') }}
                        </th>
                        <td>
                            {{ $workImage->work->title ?? '' }}
                        </td>
                    </tr>
                    <tr>
                        <th>
                            {{ trans('cruds.workImage.fields.url') }}
                        </th>
                        <td>
                            {{ $workImage->url }}
                        </td>
                    </tr>
                    <tr>
                        <th>
                            {{ trans('cruds.workImage.fields.position') }}
                        </th>
                        <td>
                            {{ $workImage->position }}
                        </td>
                    </tr>
                    <tr>
                        <th>
                            {{ trans('cruds.workImage.fields.status') }}
                        </th>
                        <td>
                            {{ App\Models\WorkImage::STATUS_SELECT[$workImage->status] ?? '' }}
                        </td>
                    </tr>
                    <tr>
                        <th>
                            {{ trans('cruds.workImage.fields.reported') }}
                        </th>
                        <td>
                            {{ App\Models\WorkImage::REPORTED_SELECT[$workImage->reported] ?? '' }}
                        </td>
                    </tr>
                </tbody>
            </table>
            <div class="form-group">
                <a class="btn btn-default" href="{{ route('admin.work-images.index') }}">
                    {{ trans('global.back_to_list') }}
                </a>
            </div>
        </div>
    </div>
</div>



@endsection