...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.workImage.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.work-images.update", [$workImage->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('work') ? 'is-invalid' : '' }}" name="work_id" id="work_id" required>
                    @foreach($works as $id => $entry)
                        <option value="{{ $id }}" {{ (old('work_id') ? old('work_id') : $workImage->work->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('work'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('url') ? 'is-invalid' : '' }}" type="text" name="url" id="url" value="{{ old('url', $workImage->url) }}" required>
                @if($errors->has('url'))
                    <div class="invalid-feedback">
                        {{ $errors->first('url') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('position') ? 'is-invalid' : '' }}" type="number" name="position" id="position" value="{{ old('position', $workImage->position) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(App\Models\WorkImage::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $workImage->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('reported') ? 'is-invalid' : '' }}" name="reported" id="reported">
                    ...
                    @foreach(App\Models\WorkImage::REPORTED_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('reported', $workImage->reported) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
