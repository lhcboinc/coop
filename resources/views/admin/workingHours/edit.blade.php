    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.working-hours.update", [$workingHour->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $workingHour->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('weekday') ? 'is-invalid' : '' }}" name="weekday" id="weekday" required>
                    ...
                    @foreach(App\Models\WorkingHour::WEEKDAY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('weekday', $workingHour->weekday) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('from') ? 'is-invalid' : '' }}" type="text" name="from" id="from" value="{{ old('from', $workingHour->from) }}" required>
                @if($errors->has('from'))
                    <div class="invalid-feedback">
                        {{ $errors->first('from') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('till') ? 'is-invalid' : '' }}" type="text" name="till" id="till" value="{{ old('till', $workingHour->till) }}" required>
                @if($errors->has('till'))
                    <div class="invalid-feedback">
                        {{ $errors->first('till') }}
                    </div>
