    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.messages.update", [$message->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('offer') ? 'is-invalid' : '' }}" name="offer_id" id="offer_id" required>
                    @foreach($offers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('offer_id') ? old('offer_id') : $message->offer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('offer'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sender') ? 'is-invalid' : '' }}" name="sender_id" id="sender_id" required>
                    @foreach($senders as $id => $entry)
                        <option value="{{ $id }}" {{ (old('sender_id') ? old('sender_id') : $message->sender->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('recipient') ? 'is-invalid' : '' }}" name="recipient_id" id="recipient_id" required>
                    @foreach($recipients as $id => $entry)
                        <option value="{{ $id }}" {{ (old('recipient_id') ? old('recipient_id') : $message->recipient->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('text') ? 'is-invalid' : '' }}" name="text" id="text" required>{{ old('text', $message->text) }}</textarea>
                @if($errors->has('text'))
                    <div class="invalid-feedback">
                        {{ $errors->first('text') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('read_at') ? 'is-invalid' : '' }}" type="text" name="read_at" id="read_at" value="{{ old('read_at', $message->read_at) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('endorsed') ? 'is-invalid' : '' }}" name="endorsed" id="endorsed">
                    ...
                    @foreach(App\Models\Message::ENDORSED_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('endorsed', $message->endorsed) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('reported') ? 'is-invalid' : '' }}" name="reported" id="reported">
                    ...
                    @foreach(App\Models\Message::REPORTED_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('reported', $message->reported) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
